...
# 注意力头的构建器
# 它的 build_model 方法解析 layer_string。当解析到 "Attention:..." 部分时，它会实例化并添加下面描述的 Block 类到其内部的 nn.Sequential 中。
class AttentionHead(nn.Module):
    ...
    def __init__(self,
                 layer_string):  # layer_string是层定义字符串，例如layer_string = f"Attention:256:8:4:{num_blocks},"，用来配置网络结构
        ...
        self.use_cls_token, dim = self.parse_use_cls_token(
            layer_string.split(',')[0].strip())  # 解析第一个层定义，获取是否使用CLS Token和CLS Token的维度
        if self.use_cls_token:  # 如果需要使用CLS Token
            self.cls_token = nn.Parameter(
                torch.zeros(1, 1, dim))  # 创建一个形状为(1, 1, dim)的零张量，第一个1表示batch size，第二个1表示序列长度，dim表示特征维度
    ...
    # build_model 会解析 layer_string
    # 当它遇到 "Attention:256:8:4:1" 这部分时，它会根据解析出的参数（dim=256, num_heads=8, mlp_ratio=4.0, num_blocks=1）创建一个 Block 类的实例。
    def build_model(self):
        ...
        for layer_definition in layer_definitions:  # 遍历每个层定义
            ...
            if layer_definition.startswith('Attention'):  # 如果层定义以'Attention'开头
                dim, num_heads, mlp_ratio, num_blocks = self.parse_block_layer(
                    layer_definition)  # 解析层定义，获取维度、注意力头数、MLP比率和块数
...
# 标准的Transformer Encoder Block，Transformer编码器
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        #
        self.drop_path = nn.Identity()  # 没有使用随机深度
        # 第四个子模块：归一化层2
        self.norm2 = norm_layer(dim)  # 创建第二个归一化层实例，通常放在 MLP 之前
        mlp_hidden_dim = int(dim * mlp_ratio)  # 计算 MLP 中间隐藏层的维度
        # 五个子模块：MLP，作为前馈网络
        # 这里的 dim 是传递给 Block 的 dim，也就是 256。
        # 计算 MLP 隐藏层维度：mlp_hidden_dim = int(dim * mlp_ratio) 即 int(256 * 4.0) = 1024。
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer,
                       drop=drop)  # 创建 Mlp 模块实例，传入输入维度、计算出的隐藏层维度、激活函数和 Dropout 概率

    def forward(self, x):
        # 定义数据如何流过这个块，输入: x，一个形状为 (Batch Size, Sequence Length, Dimension) 的张量
        # --- 第一个子层：Multi-Head Self-Attention ---
        # 1. 输入 x 首先通过第一个归一化层 (self.norm1)
        # 2. 归一化后的结果进行多头自注意力计算。序列中的每个元素会关注其他所有元素（包括自身）并计算加权表示
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        # 构造函数，接收配置参数：
        # - in_features: 输入特征的维度
        # - hidden_features: 隐藏层的特征维度 (可选，默认为 in_features)，1024
        # - out_features: 输出特征的维度 (可选，默认为 in_features)，因为 out_features 是 None，所以 out_features 被设置为 in_features 的值，即 256。
        ...
        self.act = act_layer()  # 创建激活函数实例，默认是GELU，它是由 Block 类的构造函数传递过来的。
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        assert dim % num_heads == 0  # 确保维度可以被注意力头数整除，这是多头注意力的基本要求，因为输入维度会被平均分配给各个头。
        self.num_heads = num_heads  # 将注意力头数赋值给实例变量
        self.head_dim = dim // num_heads  # 计算每个注意力头的维度
        self.scale = self.head_dim ** -0.5  # 计算缩放因子，用于防止点积结果过大，即1/根号下d_k, d_k是每个头的维度head_dim
        self.qkv = nn.Linear(dim, dim * 3,
                             bias=qkv_bias)  # 创建一个线性层，将输入维度映射到3倍的输入维度，输入维度是 dim (C)，输出维度是 dim * 3 (因为 Q, K, V 每个的维度都是 dim)。
        self.q_norm = norm_layer(
            self.head_dim) if qk_norm else nn.Identity()  # 如果 qk_norm 为 True，则创建一个 LayerNorm 层用于对 Q 进行归一化。
        self.k_norm = norm_layer(self.head_dim) if qk_norm else nn.Identity()  # 同上，为 K 创建可选的归一化层。
        self.attn_drop = nn.Dropout(attn_drop)  # 创建一个 Dropout 层，用于施加在计算出的注意力权重（Softmax 之后）上，以进行正则化。
        self.proj = nn.Linear(dim, dim)  # 创建最终的线性投影层 proj。在多头注意力的结果被合并回原始维度 dim 之后，通常会再通过一个线性层进行一次变换。
        self.proj_drop = nn.Dropout(proj_drop)  # 创建一个 Dropout 层，用于施加在最终投影层的输出上。
    ...
    def forward(self, x):
        ...
        #    把(B, N, C*3)将形状变为 (B, N, 3, H, Dh)。3 代表 Q, K, V。
        #    头数 H = self.num_heads，每个头的维度 Dh = self.head_dim。因为 3*C = 3 * H * Dh，所以元素总数保持不变。
        # 3. .permute(2, 0, 3, 1, 4):
        #    输入形状: (B, N, 3, H, Dh)。 对应维度索引: (0, 1, 2, 3, 4)。
        #    重排维度，将形状变为 (3, B, H, N, Dh)。
        #    这样做是为了方便后续将 Q, K, V 分开，并让每个头独立计算注意力。
        #    (B:批次, H:头, N:序列长度, Dh:头维度)
        q, k, v = qkv.unbind(0)  # 将 qkv 张量沿着(3, B, H, N, Dh)的第一个维度（索引为 0）进行拆分，得到三个新的张量 q, k, v，形状为 (B, H, N, Dh)。
        q, k = self.q_norm(q), self.k_norm(k)  # 对 Q 和 K 进行归一化处理。

        q = q * self.scale  # 缩放因子
        attn = q @ k.transpose(-2, -1)  # 计算注意力权重，算 Q 和 K 转置的点积（批处理矩阵乘法）,@是矩阵乘法
        # 这里先交换了k的维度，从 (B, H, N, Dh) 变为 (B, H, Dh, N)，这正是矩阵乘法中，我们需要的 Key 的转置形式 K^T
        # 矩阵乘法 (N, Dh) @ (Dh, N) 的结果是一个形状为 (N, N) 的矩阵
        # q @ k.transpose(-2, -1) 的最终输出形状是 (B, H, N, N)。这是一个注意力分数矩阵
        attn = attn.softmax(dim=-1)  # 对注意力权重进行归一化处理，对注意力分数 attn 沿着最后一个维度 (dim=-1，即Key的序列长度) 应用 Softmax 函数
        # 是因为这个维度代表了一个特定的查询 (Query) 对所有可能的键 (Key) 的注意力分数。我们的目标是将这些分数转换成一个概率分布，表示这个查询应该将多少“注意力”分配给每个对应的键/值 (Value)
        attn = self.attn_drop(attn)  # 应用 Dropout 层
        x = attn @ v  # 计算加权和，将注意力权重 attn 与 Value 矩阵 v 相乘，得到加权和 x。
        # x 的形状是 (B, H, N, Dh),attn的形状是 (B, H, N, N)，v的形状是 (B, H, N, Dh)
        x = x.transpose(1, 2).reshape(B, N, C)
        # transpose(1, 2) 交换了第二个和第三个维度，将形状变为 (B, N, H, Dh)。
        # reshape(B, N, C) 将形状变为 (B, N, C)。
...
def extract(v, t, x_shape):
    # v: 一个一维张量（Vector），包含了所有时间步对应的预计算系数值。例如，如果总步数 T=1000，v 的长度就是 1000，v[i] 存储了时间步 i 对应的系数值。
    # t: 一个一维张量，包含了当前批次中每个样本对应的时间步索引。它的形状通常是 (batch_size,)，其中每个元素是 0 到 T-1 之间的整数。
    # x_shape: 一个元组或列表，表示目标张量（通常是图像张量 x 或噪声张量 noise）的形状，例如 (batch_size, channels, height, width)
    """
    高效地提取出当前批次中每张图片在它各自对应的随机时间步 t 下所需的扩散系数（比如乘以原图的系数或乘以噪声的系数），
    并且将这些系数调整成合适的形状（比如 (80, 1, 1, 1)），使得它们能直接通过 PyTorch 的广播机制与原始图片或噪声（形状 (80, 3, 32, 32)）进行乘法运算，
    从而方便地计算出加了不同程度噪声的图片 x_t
...
class GaussianDiffusionTrainer(nn.Module):
    def __init__(self, model, beta_1, beta_T, T):  # model是U-Net模型，beta_1和beta_T是扩散过程的噪声系数初始和最终值，T是时间步长
        ...
        # 生成一个包含 T 个元素的一维张量，这些元素是从 beta_1 到 beta_T 之间等间隔分布的。这代表了扩散过程中每一步的噪声方差 beta_t。
        # 生成方差序列，包含T个元素的一维张量，并转换成双精度浮点数
        # 同时注册一个缓冲区，作为模型状态的一部分，会自动保存到模型中，不会在反向传播时计算梯度，也不会被优化器更新，命名betas
        alphas = 1. - self.betas  # 定义alpha，beatas是噪声，alphas一步扩散中保留多少原始信息
        alphas_bar = torch.cumprod(alphas,
                                   dim=0)  # 是 alphas 的累积乘积，表示从初始状态到时间步 t 的累积噪声系数, alphas_bar_t 代表从初始图像 x_0 直接扩散到 x_t 时，x_0 所乘的系数的平方根
        # 计算累积方差，用于后续的扩散过程
        ...
        # 调用之前解释过的 extract 函数。它根据随机生成的时间步 t，从预先计算好的 sqrt_alphas_bar 缓冲区中提取出对应每个样本的系数，
        # 并将其形状调整为可以与 x_0 进行广播乘法（例如，变成 (batch_size, 1, 1, 1)）。
        # 同样，根据时间步 t 提取对应的 sqrt(1 - alphas_bar_t) 系数，并调整形状。

    def forward(self, x_0):  # 模型的前向传播过程，实现了 DDPM 的训练算法
        t = torch.randint(self.T, size=(x_0.shape[0],), device=x_0.device)  # noise 是一个与 x_0 形状相同的张量，包含从标准正态分布中采样的噪声
        # 生成一个包含 batch_size 个随机整数的张量 t。每个整数的范围是从 0 到 self.T - 1（包含 0，不包含 self.T）。
        # device=x_0.device 确保生成的随机时间步 t 与输入数据 x_0 在同一个设备上（CPU 或 GPU）。这个 t 张量为批次中的每个样本随机选择了一个扩散时间步。
        noise = torch.randn_like(x_0)  # noise 是一个与 x_0 形状相同的张量，包含从标准正态分布中采样的噪声
        x_t = (
                extract(self.sqrt_alphas_bar, t, x_0.shape) * x_0 +
                extract(self.sqrt_one_minus_alphas_bar, t, x_0.shape) * noise)
        # 整个表达式计算出了根据扩散公式 x_t = sqrt(alphas_bar_t) * x_0 + sqrt(1 - alphas_bar_t) * noise 得到的加噪后的图像 x_t。
        # 对于批次中的每个样本，都使用了其对应随机时间步 t 的系数。
        loss = F.mse_loss(self.model(x_t, t), noise, reduction='none')  # 计算预测的噪声和实际噪声之间的均方误差损失
        # self.model(x_t, t): 将加噪后的图像 x_t 和对应的时间步 t 输入到 U-Net 模型 (self.model) 中。模型的任务是预测出添加到 x_0 上的噪声，也就是我们之前生成的 noise。
        return loss  # 回计算得到的损失张量。这个损失将在训练循环中用于计算梯度和更新 U-Net 模型的参数，目标是让模型预测的噪声越来越接近实际添加的噪声。


# 分类器的实现
class GaussianDiffusionClassifyTrainer(nn.Module):
    def __init__(self, model, beta_1, beta_T, T, head,
                 classify_T):  # model是U-Net模型，beta_1和beta_T是扩散过程的噪声系数初始和最终值，T是时间步长，head是分类头，classify_T是分类时间步长
        super().__init__()
        self.head = head  # 分类头
        ...
        # 全连接层
        if self.head == 'fc':
            self.pool = nn.AdaptiveAvgPool2d((4, 4))  # 自适应平均池化层，将输入特征图的空间维度（高和宽）调整为精确的4*4，通道数不变，进行特征降维，提取关键空间信息
        ...
        # 多层感知机
            ...
            self.fc = nn.Sequential(  # 定义了一个构成MLP的层序列
                ...
                nn.BatchNorm1d(256),
                # 对第一个线性层的输出应用 1D 批归一化,稳定和加速训练过程，全连接层的输出是[batch_size，num_featuers]的二维张量，在num_featuers这个维度上进操作，计算均值和方差并进行归一化
                nn.ReLU(inplace=True),  # 激活函数引入非线性,直接在原始输入张量（Tensor）的内存上进行计算并覆盖结果，而不是创建一个新的张量来存储结果。
                nn.Linear(256, 64),  # 第二个线性层，从 256 维降到 64维
                nn.BatchNorm1d(64),  # 1D 批归一化
                nn.ReLU(inplace=True),  # ReLU 激活函数
                nn.Linear(64, 10)  # 输出线性层，从 64维映射到 10个类别得分
            )  # 78.20

        elif self.head == 'attn':
            ### attention
            self.rer = LambdaLayer(lambda x: rearrange(x, 'b c h w -> b (h w) c'))
            # 创建一个预处理层，用于将从 U-Net 提取的特征图转换为 Transformer 模型能够处理的序列格式。
            # 这个 lambda 函数的核心是 einops.rearrange，输入 x 的形状被假定为 (batch_size, channels, height, width)，输出形状变为 (batch_size, sequence_length, channels)。
            # h (height) 和 w (width) 这两个空间维度被合并成一个新的序列维度 N = height * width。每个原始空间位置 (h, w) 对应序列中的一个元素。
            # 将 U-Net 输出的空间特征图“拉平”成一个序列，序列中的每个元素代表原图的一个空间位置（或区域）的特征向量（维度为 c)
            num_blocks = 1  # 这个注意力头将只包含一个标准的 Transformer 编码器层
            layer_string = f"Attention:256:8:4:{num_blocks},"
            # 创建一个配置字符串，用于告诉 AttentionHead 类如何构建其内部的网络结构。
            # "Attention": 指定要创建一个或多个 Block 模块 (Transformer 块)。
            # "256": 指定这些 Block 的输入/输出特征维度 (dim)。这通常需要与从 U-Net 提取的特征图的通道数相匹配。
            # "8": 指定每个 Block 内部 Attention 模块使用的注意力头数 (num_heads)。
            # "4": 指定每个 Block 内部 Mlp 模块的隐藏层维度相对于输入维度的比例 (mlp_ratio)。隐藏层维度将是 256 * 4 = 1024。
            # f"{num_blocks}" (即 "1"): 指定要创建的 Block 模块的数量。
            # 创建实际的分类头模块。
    ...
    def forward(self, x_0, labels):
        ...
        # 创建一个张量 t，其大小与批次大小 (x_0.shape[0]) 相同。
        # 这个张量的所有元素都被填充为固定的分类时间步 self.classify_T。dtype=torch.long 确保是整数类型，device=x_0.device 确保在同一设备上。
        noise = torch.randn_like(x_0)  # 生成一个与输入 x_0 形状相同的张量，填充从标准正态分布（均值为0，方差为1）中采样的随机数。这是要添加到图像上的噪声。
        ...
        if self.head == 'fc':
            ...
            feature = self.pool(features[0])  # 自适应平均池化层
            ...
            # 输出 `pre` 的形状可能是 [B, N, 10] (如果对序列中每个位置都预测) 或 [B, 10s] (如果使用了 CLS Token 并提取了其输出)。
            pre = pre.mean(dim=1).squeeze(1)
            pre = self.classifier(pre)
            # 将平均后的特征通过分类器映射到10个类别
        ...
        ### 4.CNN
            ...
            # 初始输入: feature 形状为 [B, 10, 3, 3]
            # B: 批量大小
            # 10: 通道数（对应10个类别）
            # 3×3: 空间尺寸
            # 第一步平均: feature.mean(dim=3)
            # 对宽度维度（最后一个维度，索引3）求平均
            # 结果形状: [B, 10, 3]
            # 第二步平均: .mean(dim=2)
            # 对高度维度（索引2）求平均
            # 最终形状: [B, 10]
