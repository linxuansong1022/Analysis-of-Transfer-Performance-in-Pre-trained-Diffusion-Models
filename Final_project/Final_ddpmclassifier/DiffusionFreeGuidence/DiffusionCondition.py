...
class GaussianDiffusionClassifyTrainer(nn.Module):
    ...
    def forward(self, x_0, labels):
        ...
        features = self.model.get_multiple_features(x_t, t, labels)
        #print(len(features),features[0].shape) [80, 256, 4, 4]
        feature = self.pool(features[0]) 

        # features = self.model(x_0, t, labels)
        # feature = self.pool(features) 
