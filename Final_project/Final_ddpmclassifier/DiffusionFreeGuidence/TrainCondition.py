...
from DiffusionFreeGuidence.DiffusionCondition import GaussianDiffusionSampler, GaussianDiffusionTrainer,GaussianDiffusionClassifyTrainer
...
def train_classify(modelConfig: Dict):
    ...
    net_model = UNet(T=modelConfig["T"], num_labels=10, ch=modelConfig["channel"], ch_mult=modelConfig["channel_mult"],
                     num_res_blocks=modelConfig["num_res_blocks"], dropout=modelConfig["dropout"]).to(device)
    if modelConfig["training_load_weight"] is not None:
        net_model.load_state_dict(torch.load(modelConfig["training_load_weight"], 
    ...
    #head = Head(DM_FEAT_DIM_DICT,DM_FEAT_SIZE_DICT)
    #mode = 'freeze'
    # classify_model = DiffSSLModelFeedbackFusion(net_model, None, head, 
    ...
    #                 first_fw_b_list=[24],
    #                 second_fw_b_list=-1,
    #                 use_feedback=False).to(torch.device("cuda"))

    
    # optimizer = torch.optim.AdamW(
    #     net_model.parameters(), lr=modelConfig["lr"], weight_decay=1e-4)
    trainer = GaussianDiffusionClassifyTrainer(net_model, modelConfig["beta_1"], modelConfig["beta_T"], modelConfig["T"]).to(device)
    optimizer = torch.optim.AdamW(
    ...
    for e in range(modelConfig["epoch"]):
        ...
        with tqdm(train_dataloader, dynamic_ncols=True) as tqdmDataLoader:
            for images, labels in tqdmDataLoader:
                # train
                #print(images.shape) torch.Size([80, 3, 32, 32]) 
                #print(labels)
                b = images.shape[0]
                optimizer.zero_grad()
                x_0 = images.to(device)
